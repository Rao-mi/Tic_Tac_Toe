...
bool line_check (std::vector<state>& grid, int cell_number, int dimension, int line, int start, std::vector<std::vector<int>> constrain) {
    int u = line;
    int flag=1;
    ...
    for (int i=0; i<tot_dim; i++) {
            ...
            n_directions = numeric_directions(cell_number,directions); // Trasformo i vettori in numeri
                       for (size_t k=0; k<n_directions.size(); k++){
                            if (grid[i+n_directions[k]]==player && line_check(grid,cell_number,dimension,n_directions[k],i, limit)) //qui faccio check della direzione partendo dalla cella grid[i+n_directions[k]], solo se player!=N
                            {
                                return true;
                            }}
